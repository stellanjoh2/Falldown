...
export function mountTooltips(root: ParentNode): void {
  ...
  const entering = (el: HTMLElement, related: EventTarget | null) => {
  ...
  const leaving = (el: HTMLElement, related: EventTarget | null) => {
    if (related instanceof Node && el.contains(related)) return;
    if (related instanceof Element && tipTarget(related, root) === el) return;
